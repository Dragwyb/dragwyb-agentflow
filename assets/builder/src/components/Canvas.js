	...
	return (
		...
		>
				...
				>
					...
					{flowNodes.slice(0, -1).map((node, index) => {
						...
						const y1 =
							...
							(isAgentNode(node) ? AGENT_TOTAL_HEIGHT : NODE_HEIGHT);
			...
			{nodes
				...
				.filter((node) => isChatModelAttachment(node) && node.parent_agent_id)
			...
			{nodes
				...
				.filter((node) => isMemoryAttachment(node) && node.parent_agent_id)
			...
			{nodes
				...
				.filter((node) => isToolAttachment(node) && node.parent_agent_id)
