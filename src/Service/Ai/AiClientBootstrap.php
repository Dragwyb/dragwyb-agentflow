...
namespace AIAWA\Plugin\Service\Ai;

use AIAWA\AiProviders\DeepSeek\DeepSeekProvider;
use AIAWA\AiProviders\Groq\GroqProvider;
use AIAWA\AiProviders\OpenRouter\OpenRouterProvider;
...
class AiClientBootstrap {
	...
	public const MIGRATION_OPTION = 'aiawa_ai_credentials_migrated_to_wp70';

	/**
	 * Provider id map: aiawa slug → AiClient / Connectors id.
	 *
	...
	/**
	 * Integration slugs on aiawa_connections that hold AI API keys.
	 *
	 * @var array<string, string>
	 */
	...
	public static function boot(): void {
		...
		if ( ! $is_wp70 ) {
			$sdk_autoload = AIAWA_PLUGIN_DIR . 'vendor/wordpress/wp-ai-client/autoload.php';
		...
		$providers_autoload = AIAWA_PLUGIN_DIR . 'includes/ai-providers/vendor/autoload.php';
	...
	/**
	 * Map aiawa provider slug to AiClient provider id.
	 */
	public static function resolveProviderId( string $aiawa_provider ): string {
		$key = strtolower( trim( $aiawa_provider ) );
	 ...
	 */
	public static function isProviderConfigured( string $aiawa_provider ): bool {
		if ( ! self::isAvailable() ) {
			return false;
		}

		if ( ! self::hasStoredProviderApiKey( $aiawa_provider ) ) {
			return false;
		}

		self::ensureProviderAuthentication( $aiawa_provider );

		$provider_id = self::resolveProviderId( $aiawa_provider );
	...
	public static function usesCoreConnectors(): bool {
		...
		return version_compare( aiawa_wp_version(), '7.0-alpha', '>=' );
	}
	 ...
	 */
	public static function getStoredApiKey( string $aiawa_provider ): string {
		$provider_id = self::resolveProviderId( $aiawa_provider );
		if ( '' === $provider_id ) {
			return '';
		}
	 ...
	 */
	public static function ensureProviderAuthentication( string $aiawa_provider ) {
		if ( ! self::isAvailable() ) {
			return new WP_Error(
	 ...
	 *
	 * @param string $aiawa_provider Provider slug (openai, claude, openrouter, …).
	 * @param string $api_key      Raw API key.
	 *
	 * @return true|WP_Error
	 */
	public static function saveProviderApiKey( string $aiawa_provider, string $api_key ) {
		if ( ! self::isAvailable() ) {
			return new WP_Error(
				'aiawa_ai_unavailable',
				__( 'WordPress AI Client is not available.', 'dragwyb-agentflow' ),
		...
		delete_transient( 'aiawa_ai_models_' . $provider_id );
	 ...
	 */
	public static function hasStoredProviderApiKey( string $aiawa_provider ): bool {
		return '' !== self::getStoredApiKey( $aiawa_provider );
	}

	/**
	 * Remove a site-wide API key for a provider.
	 *
	 * @param string $aiawa_provider Provider slug.
	 *
	 * @return true|WP_Error
	 */
	public static function clearProviderApiKey( string $aiawa_provider ) {
		if ( ! self::isAvailable() ) {
			return new WP_Error(
				'aiawa_ai_unavailable',
				__( 'WordPress AI Client is not available.', 'dragwyb-agentflow' ),
	...
	/**
	 * One-time migration: aiawa AI connections → connectors_ai_* on WP 7+.
	 */
	...
	/**
	 * One-time migration: aiawa AI connections → wp_ai_client_provider_credentials below WP 7.
	 */
	private static function migrateCredentialsToLegacyOption(): void {
		if ( get_option( 'aiawa_ai_credentials_migrated_to_sdk' ) ) {
			return;
		}
		...
		update_option( 'aiawa_ai_credentials_migrated_to_sdk', true );
	}
	...
	private static function migrateFromConnectionsTable( callable $writer ): void {
		...
		try {
			$plugin = \AIAWA\Plugin\Core\Plugin::instance();
