...
class AgentToolExecutor {
	...
	public function execute(
		...
		if ( 'condition_action' === $node_type ) {
			$result = $this->annotateConditionResult( $result, $graph_nodes );
		} elseif ( 'router_action' === $node_type ) {
			$result = $this->annotateRouterResult( $result, $graph_nodes );
		}
	...
	/**
	 * @param array<string, mixed> $result      Condition tool result.
	 * @param array<int, mixed>    $graph_nodes Graph nodes.
	 *
	 * @return array<string, mixed>
	 */
	private function annotateConditionResult( array $result, array $graph_nodes ): array {
		$passed    = ! empty( $result['passed'] );
		$branch_id = $passed
			? (string) ( $result['true_branch_node_id'] ?? '' )
			: (string) ( $result['false_branch_node_id'] ?? '' );

		if ( '' === $branch_id ) {
			return $result;
		}

		$branch_node = AgentGraphHelper::findNode( $graph_nodes, $branch_id );

		if ( null === $branch_node ) {
			return $result;
		}

		$result['next_tool'] = AgentToolSchemaBuilder::toolName(
			(string) ( $branch_node['type'] ?? '' ),
			$branch_id
		);
		$result['next_tool_label'] = isset( $branch_node['label'] ) ? (string) $branch_node['label'] : (string) ( $branch_node['type'] ?? '' );

		return $result;
	}

	/**
	 * @param array<string, mixed> $result      Router tool result.
	 * @param array<int, mixed>    $graph_nodes Graph nodes.
	 *
	 * @return array<string, mixed>
	 */
	private function annotateRouterResult( array $result, array $graph_nodes ): array {
		$branch_id = (string) ( $result['branch_node_id'] ?? '' );
