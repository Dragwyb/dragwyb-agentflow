...
final class TaxonomyActionCatalog {
	...
	public static function definitions( callable $field, callable $option, array $groups ): array {
		...
		$definitions[] = array(
			...
			'method_args' => array(),
			'config_schema' => array(
				'url' => $field( 'string', __( 'Image URL', 'workflow-automate' ), array( 'required' => true, 'description' => __( 'Direct https image URL (.jpg/.png/.webp). Do not use example.com or search-page URLs.', 'workflow-automate' ) ) ),
		...
		$definitions[] = array(
			...
			'method_args' => array(),
			'config_schema' => array(
				'limit' => $field( 'integer', __( 'Limit', 'workflow-automate' ), array( 'default' => 50, 'description' => __( 'Max items (default 50, max 200).', 'workflow-automate' ) ) ),
			),
		);
