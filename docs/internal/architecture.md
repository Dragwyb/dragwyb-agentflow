...
- Custom capabilities for workflow management, layered on top of a `manage_options` fallback (opportunity #7) — implemented in roadmap item 14 as `Core\Capabilities`: granular caps (`wfa_manage_workflows`, `wfa_manage_runs`, `wfa_manage_connections`, `wfa_manage_webhooks`, `wfa_manage_settings`) plus `wfa_access` for menu visibility; granted to the `administrator` role on activation; a `user_has_cap` filter also grants every plugin cap to anyone who already has `manage_options`, so existing admins need no role-editor work. Call sites check the granular caps only — never `manage_options` directly.
