...
`db_version`, `installed_at`, `global_settings` (see §2.5 for its actual keys, as implemented in roadmap item 10 — this entry originally also anticipated a "notification email" key, which was not built; see that section's implementation note for why), `remove_data_on_uninstall` (roadmap item 1; a settings-screen UI for it shipped with item 10), `encryption_key_id` (see Security). No option ever stores raw secrets in plaintext.
...
> - **Security/API Keys:** not built. This tab is explicitly connection management, which does not exist until roadmap item 11 — building an entry point to nothing would be the same premature-surface mistake items 5/6/9 already deliberately avoided elsewhere in this project.
